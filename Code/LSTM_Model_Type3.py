...
def LSTM_FitPredict(sa3_codes, population_dict, age_groups_labels, n_steps, train_val_bounds, test_bounds, n_features, epochs_num, sex_label, pred_start, tuner, output):
    ...
    for code in sa3_codes:
        ...
        age_sample_data = []
        # Fit the model with each age range in the selected area
        for i in range(18):
            for year_data in male_sample_data:
                age_sample_data.append(year_data[i])                    
                                       
            X, y = split_sequence(age_sample_data, n_steps)
